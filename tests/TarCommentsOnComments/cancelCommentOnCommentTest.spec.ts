...
test('cancel comment on comment test', async ({ Commander: page }) => {
  ...
  await tar.clickOnFirstAddCommentOnCommentButton();
  await tar.enterCommentOnComment('הערה על הערה אוטומטית')
  await tar.clickOnCancelCommentOnCommentButton();
});
